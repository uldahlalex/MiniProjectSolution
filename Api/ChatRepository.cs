...
{
    public Object InsertMessage(InsertMessageParams message)
    {
        using (var conn = new NpgsqlConnection(dataSource.ConnectionString))
        {
            return conn.Execute("INSERT INTO chat.messages (messagecontent, sender, timestamp, room) VALUES (@messagecontent, @sender, @timestamp, @room)", message);
        }
    }
